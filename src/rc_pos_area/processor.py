    ...
    try:
        ...
        try:
            ...
            if 2 not in thresholds_df["Channel #"].values:
                errors.append(
                    "Channel #2 is required for tissue region definition but not found in 'Thresholds' sheet"
                )
                return errors
            ...
            for idx, row in thresholds_df.iterrows():
                ...
                if pd.isna(channel_num):
                    errors.append(
                        f"Row {idx + 2} in 'Thresholds' sheet has empty Channel #"
                    )
                elif not isinstance(channel_num, (int, np.integer)):
                    errors.append(
                        f"Row {idx + 2} in 'Thresholds' sheet has non-integer Channel # ({channel_num})"
                    )
                elif channel_num < 1 or channel_num > num_channels:
                    errors.append(
                        f"Channel # {channel_num} is invalid (image has {num_channels} channels: 1-{num_channels})"
                    )
...
def process_single_excel(
    excel_path: Path, output_dir: Path, verbose: bool = False, quiet: bool = False
) -> Path:
    """
    ...
    for cc, tt in list(thresholds_df["Threshold"].items()):
        ...
        thresholds_df.loc[cc, "Area P in T (µm^2)"] = (
            int(np.sum(p_in_t_mask)) * PIXEL_SIZE**2
        )
        del gimg_np
    ...
    intensity_cols = [
        "Mean Intensity P in T",
        "Median Intensity P in T",
        "Mean Intensity T excl P",
        "Median Intensity T excl P",
        "Mean Intensity BG",
        "Median Intensity BG",
    ]
